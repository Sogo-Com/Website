  ...
  do {
    ...
    $$rendered = `${validate_component(Layout, "Layout").$$render($$result, {}, {}, {
      ...
      default: () => {
		...
		<form method="POST" action="?/create" class="actualite-form svelte-dxsg85"><div class="form-group svelte-dxsg85"><label for="titre" class="svelte-dxsg85">Titre</label>
				<input id="titre" name="titre" contenteditable="true" type="text" class="svelte-dxsg85"${add_attribute("value", actualite.titre, 0)}></div>

			<div class="form-group svelte-dxsg85"><label for="file" class="svelte-dxsg85">Image</label>
				<input type="file" id="file" name="photoFile"${add_attribute("accept", [".jpg", ".jpeg", ".png", ".webp"].join(","), 0)}>
				${actualite.photo != null && actualite.photo.length != 0 ? `<img${add_attribute("src", actualite.photo, 0)}${add_attribute("alt", actualite.titre, 0)} class="svelte-dxsg85">` : ``}</div>


			<div class="form-group svelte-dxsg85"><label for="redacteur" class="svelte-dxsg85">Rédacteur</label>
				<input id="redacteur" name="redacteur" contenteditable="true" type="text" class="svelte-dxsg85"${add_attribute("value", actualite.redacteur, 0)}></div>
			<div class="form-group svelte-dxsg85"><label for="tempsLecture" class="svelte-dxsg85">Temps de Lecture (en minutes)</label>
				<input id="tempsLecture" name="tempsLecture" contenteditable="true" type="number" class="svelte-dxsg85"${add_attribute("value", actualite.tempsLecture, 0)}></div>

			<div class="form-group svelte-dxsg85"><label for="descriptionCourte" class="svelte-dxsg85">Description courte (200 caracteres max)</label>
				<textarea id="descriptionCourte" rows="3" cols="45" maxlength="200" name="descriptionCourte" contenteditable="true" type="text" class="svelte-dxsg85">${actualite.descriptionCourte || ""}</textarea></div>

			<div class="form-group svelte-dxsg85"><label for="contenu" class="svelte-dxsg85">Contenu</label>
				
				<div id="contenu" class="contenu svelte-dxsg85" name="contenu" required>${validate_component(Writer, "Writer").$$render(
          $$result,
          {
          ...
          {}
			...
			<button type="submit" for="envoyer" value="envoyer" class="submit-button svelte-dxsg85">Enregistrer</button></form></div>`;
      }
    })}`;
  } while (!$$settled);
  return $$rendered;
...
//# sourceMappingURL=_page@.svelte-c4d81b05.js.map
