...
<Layout>
	<div slot="buttons">
		<a href="/admin/icon" class="create-button" ><span>Créer un nouvel icon</span></a>
	</div>
	<div class="grid-view">
		<h1>
			{expertiseIcons.length == 0 ? 'Aucun icon' : 'Liste des icons d\'expertise'}
		</h1>

		<table class="expertiseIcon-table" style={expertiseIcons.length == 0 ? 'display:none;' : ''}>
			<thead>
				<tr>
					<th>Titre</th>
					<th>Onglet</th>
			...
			<tbody>
				{#each expertiseIcons as expertiseIcon, index}
					<tr>
						<td
...
<style lang="scss">
	...
	.grid-view {
		...
		.grid-item {
			...
			/* Styles spécifiques pour le contenu de la demande de expertiseIcon */
